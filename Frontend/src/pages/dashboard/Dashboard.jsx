...
import { apiGetVehicles, apiDeleteVehicle, mapVehicleFromApi } from '../../utils/api';
                ...
                {[...userRoutes].reverse().map(route => (
                  <div key={route.id} className="route-card" onClick={() => navigate(`/route/${route.id}`)}>
                    <div className="route-card-header">
                      ...
                      <button className="route-delete" onClick={e => {
                        e.stopPropagation();
                        if (window.confirm('Delete this route?')) {
                          const all = JSON.parse(localStorage.getItem('ev_routes') || '[]');
                          localStorage.setItem('ev_routes', JSON.stringify(all.filter(r => r.id !== route.id)));
