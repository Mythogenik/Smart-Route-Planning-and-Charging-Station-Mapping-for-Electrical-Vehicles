...
import { apiGetVehicles, apiDeleteVehicle, mapVehicleFromApi, apiGetRoutes, apiDeleteRoute, mapRouteFromApi, apiGetMe } from '../../utils/api';
...
export default function Dashboard() {
  ...
  const { currentUser, logout, updateRoutesRemaining } = useAuth();
  ...
  useEffect(() => {
    apiGetMe().then(data => {
      if (data?.routesRemaining !== undefined) {
        updateRoutesRemaining(data.routesRemaining);
      }
    }).catch(() => { });
  }, []);
  ...
  async function removeCar(id) {
    ...
    const affectedRoutes = userRoutes.filter(r => r.car?.make === car?.make && r.car?.model === car?.model);

    const message = affectedRoutes.length > 0
      ? `Remove "${car?.nickname || `${car?.make} ${car?.model}`}"?\n\n⚠️ This will also delete ${affectedRoutes.length} route${affectedRoutes.length > 1 ? 's' : ''} that use this vehicle.`
      : `Remove "${car?.nickname || `${car?.make} ${car?.model}`}"?`;
    ...
    try {
      ...
      if (affectedRoutes.length > 0) {
        setUserRoutes(prev => prev.filter(r => !(r.car?.make === car?.make && r.car?.model === car?.model)));
      }
    } catch (err) {
          ...
          <div className="content">
            {userRoutes.length === 0 ? (
              <div className="empty-panel">
                ...
                <button className="btn-create-route-lg" onClick={handleCreateRoute}>+ CREATE YOUR FIRST ROUTE</button>
              </div>
            ) : (
              ...
              <div className="account-fields">
                ...
                <div className="account-field">
                  ...
                  <span className="account-field-value account-field-green">
                    {routesRemaining === -1 ? 'Unlimited' : `${routesDisplay} route${routesDisplay !== 1 ? 's' : ''} left`}
                  </span>
                </div>
                <div className="account-field">
                  <span className="account-field-label">Plan</span>
                  <span className="account-field-value">
                    {routesRemaining === -1 ? 'Unlimited' : 'Pay-as-you-go'}
                  </span>
                </div>
              </div>
              <button
      ...
      {showRouteGate && (
        <div className="gate-overlay" onClick={() => setShowRouteGate(false)}>
          <div className="gate-modal" onClick={e => e.stopPropagation()}>
            ...
            <h2 className="gate-title">No Routes Remaining</h2>
            <p className="gate-desc">
              You've used all your available routes. Top up to continue planning trips.
            </p>
            <div className="gate-actions">
              <button className="gate-btn-secondary" onClick={() => setShowRouteGate(false)}>
                Maybe Later
              </button>
              <button className="gate-btn-primary" onClick={() => { setShowRouteGate(false); navigate('/payment'); }}>
                ⚡ Top Up Routes
              </button>
            </div>
          </div>
